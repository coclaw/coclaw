...
function mockRepo(overrides = {}) {
	return {
		...
		topActiveUsers: async () => overrides.topActive ?? [
			{ id: '1', name: 'Alice', lastLoginAt: '2026-03-23T10:00:00Z', botCount: 2, onlineBotIds: ['10', '11'] },
		],
...
test('getAdminDashboard: 自定义数据正确透传', async () => {
	const result = await getAdminDashboard({
		repo: mockRepo({ total: 50, todayNew: 2, todayActive: 10, botsTotal: 0, todayNewBots: 0, topActive: [], latestRegistered: [], botList: [] }),
