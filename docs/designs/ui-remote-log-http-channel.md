...
| 408 / 429 | 重试，遵循 `Retry-After`（如有） |
| 5xx | 重试 |
| network error / timeout | 重试 |

退避：指数 1s → 2s → 4s → 8s → 16s → ... → 上限 60s，加小幅随机抖动。抖动只延后 in-flight 那一批，不参与排序，不会破坏顺序发送约束。

终止条件：**单 batch 最多重试 8 次或总尝试时长 10 分钟**，二者先到为准；超限丢弃。

**失败窗口容量估算**：worst case 下 server 长期不可达 10 分钟，期间 UI 仍以 5 秒 / 100 条节奏封批：
