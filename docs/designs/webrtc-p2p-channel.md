...
依据：
1. Phase 4 实施 TURN over TLS on 443 后，RTC 不可用的概率降至极低（仅白名单出站或 MITM 代理环境）
2. 这些极端网络环境同样会阻断 WS（相同的限制策略），因此 WS-only 场景本身就极为罕见
3. 在如此低概率的场景下实现 WS 文件传输（需处理分片、流控、大文件等复杂逻辑）投入产出比极低
4. 即便 RTC 走 TURN TCP 中继兜底，SCTP 的消息分帧机制仍优于 WS 的字节流——分片传输不会造成队头阻塞，RTC 保活和 ICE restart 等控制消息可正常交付

处理方式：检测到 `transportMode === 'ws'` 时，文件发送入口置灰并提示用户当前网络环境不支持文件传输。文本消息和其他 RPC 交互不受影响。
...
| Plugin ICE candidates 先于 answer 到达 | UI 侧缓存到 `__pendingCandidates`，`setRemoteDescription` 完成后批量 `addIceCandidate`。werift 的 ICE 收集与 answer 发送并行，候选常先于 answer 到达 |
