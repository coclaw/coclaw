...
| Phase 4 | TURN over TLS on 443（详见 [`turn-over-tls.md`](turn-over-tls.md)） | **已完成** |

### Phase 4 概要

**TURN over TLS on 443**：将 TURN 流量通过 TLS 包裹在 443 端口传输，使其与 HTTPS 流量无法区分，穿透绝大多数限制性网络。采用双公网 IP 方案（nginx 和 coturn 各绑独立 IP），避免端口冲突和 nginx stream 的复杂度。coturn 使用独立域名 `edge.coclaw.net`。

ICE 框架自动按优先级尝试所有路径（P2P → TURN UDP → TURN TCP → TURNS 443），应用层无需干预。

完整方案、实施细节、踩坑记录详见 [`turn-over-tls.md`](turn-over-tls.md)。
