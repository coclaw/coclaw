...
CoClaw UI 需要对 OpenClaw Agent 工作区中的文件进行读/写/删除/列表操作。WebRTC P2P DataChannel 基础设施提供了 UI ↔ Plugin 的直连通道，文件操作将基于此通道实现。

### 目标

1. UI 能对指定 Agent 的 workspace 目录进行文件操作（list / read / write / delete）
...
| 通道 | 类型 | 职责 |
|------|------|------|
| `rpc` DataChannel | 持久，JSON | 无数据传输的文件操作（list / delete）及 Gateway RPC 转发 |
| `file:<transferId>` DataChannel | 临时，String + Binary | 自包含的文件传输（read / write），DC 内完成元信息交换与数据传输 |

### 设计选择

采用**自包含 DataChannel**方案（类 HTTP 语义映射）。这是 WebRTC DataChannel 文件传输的社区主流做法。每次文件传输创建一条独立 DC，在 DC 内自包含完成请求-响应的完整生命周期，不依赖 `rpc` 通道。
...
- `rpc` 通道职责纯净：仅承载无数据传输的操作和 Gateway RPC 转发
...
将 HTTP 的请求-响应模型映射到 DataChannel 上。DC 的 `onmessage` 天然区分 `string`（`typeof data === 'string'`）和 `binary`（`ArrayBuffer / Buffer`），利用这一特性实现帧类型分离：
...
- **文件覆盖**：默认静默覆盖已存在的文件（与 HTTP PUT 语义一致）
- **目录自动创建**：写入路径中的中间目录不存在时，自动创建（类似 `mkdir -p`）
...
|------|----------------------------------------|
| 下载成功 | → request → ← response header → ← binary×N → ← completion{ok} → close |
| 下载失败（校验阶段） | → request → ← error → close |
| 下载失败（传输中途） | → request → ← response header → ← binary×? → ← error → close |
| 上传成功 | → request → ← ready → → binary×N → → done → ← result{ok} → close |
| 上传失败（校验阶段） | → request → ← error → close |
| 上传失败（写入阶段） | → request → ← ready → → binary×N → → done → ← result{error} → close |
| 上传失败（超限） | → request → ← ready → → binary×? → ← error{SIZE_EXCEEDED} → close |
...
`list` 和 `delete` 不涉及数据传输，直接走 `rpc` DataChannel 上的标准 JSON-RPC。
...
rpc DataChannel onmessage
  ↓ parse JSON
  ↓
  method.startsWith("coclaw.file.") ?
    ├── YES → FileHandler 本地处理（直接 fs 操作）
    └── NO  → 转发至 gatewayWs（现有 RPC 逻辑）
```

### 5.1 coclaw.file.list

列出目录内容（单层，不递归）。

```js
// Request
{ type: "req", id: "r1", method: "coclaw.file.list",
...
### 5.2 coclaw.file.delete

删除文件（或空目录）。

```js
// Request
{ type: "req", id: "r2", method: "coclaw.file.delete",
...
| 错误码 | 含义 | 出现位置 |
|--------|------|---------|
| `NOT_FOUND` | 文件/目录不存在 | rpc response / file DC |
| `PATH_DENIED` | 路径穿越 workspace 边界 | rpc response / file DC |
| `AGENT_DENIED` | agentId 未绑定到当前 bot | file DC |
| `SIZE_EXCEEDED` | 文件超过 1GB 上传限制（含接收端实际字节数超限） | file DC |
| `IS_DIRECTORY` | 对目录执行了文件操作（如 read） | rpc response / file DC |
| `NOT_EMPTY` | 删除非空目录 | rpc response |
...
UI 通过累计已发送/已接收字节数与总大小计算进度百分比。不需要协议层支持——总大小已在请求（write 的 `size`）或响应头（read 的 `size`）中告知。
