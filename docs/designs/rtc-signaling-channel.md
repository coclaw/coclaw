...
```
network:online → __handleNetworkOnline(typeChanged)
  │
...
| 7b | network:online + 类型变化 | 直接 rebuild 所有 claw（旧 ICE 路径失效） |
| 7c | network:online + 类型未变 + PC failed/closed | 直接 rebuild（加速长 offline 后恢复） |
| 7d | network:online + 类型未变 + PC connected/disconnected | 跳过，信任 ICE 自检测 |
