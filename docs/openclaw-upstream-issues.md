...
| [#84209](https://github.com/openclaw/openclaw/issues/84209) | Feature | Persist sessionKey in session transcript JSONL header (orphan recovery + plugin identity) | Open | 2026-05-19 | 当前 `.jsonl` 首行 header 只含 `type/version/id/timestamp/cwd`，没有 sessionKey；plugin 不得不自维护 `coclaw-chat-history.json` 把 sessionKey↔sessionId 映射挂起来。前序 #55849/#55993 被 clawsweeper 同日互判 duplicate 一起关闭、PR #56082 实现 closed unmerged。issue 已重提并串联完整历史。落地后可下放 chat-history 映射文件 |
| [#88392](https://github.com/openclaw/openclaw/issues/88392) | Feature | Expose per-model availability (the `available` flag) to plugins and RPC clients | Open | 2026-05-31 | CoClaw `coclaw.model.listUsable` 选模型器拿不到 OpenClaw 权威可用清单（`ModelRegistry.getAvailable()` 的 `available` 字段，仅 CLI `models list --all` 暴露）；plugin-sdk 未导出 `getAvailable`/`createModelListAuthIndex.hasProviderAuth`，`models.list` RPC 不带该字段。两个备选方案：①给 `models.list` RPC 加 `available` 字段（首选，读结果即可）②plugin-sdk 导出 getAvailable/auth-index。**这是插件侧"照搬可用清单"修复的前置依赖**——落地前 CoClaw 只能用 provider 级近似 workaround。详见 `tmp/codex-oauth-model-picker-fix--clear-dump.md` |
