...
- [消息 ID 通道与 Transcript 推送](openclaw-research/message-id-and-transcript-events.md) — JSONL 8-char id 暴露机制、session.message 推送事件、各 RPC 路径能力差异、引入版本时间线
...
- [Provider 清单/Plan 计费/认证区分](openclaw-research/provider-catalog-plan-and-auth.md) — 三套 provider 清单口径差异、各厂商订阅套餐现实、token/plan 认证区分机制与 CoClaw 三环缺口、'plan' 徽章可行性
- [pion-ipc 接口过滤红线](openclaw-research/pion-interface-filter-hasprefix.md) — `HasPrefix` 黑/白名单的常见误杀点、推荐默认配置
