...
- **UI 层抽象**：`coclaw-file://botId:agentId/path` URL 协议，提供连接无关的文件标识与按需获取能力（详见 `designs/file-management.md` 第七章）

---

## 三、BotConnection 抽象

`BotConnection` 是业务层与通信层的唯一接口，每个 claw 对应一个实例。
...
> 注：UI 层的 `BotConnection`、`useBotConnections()`、`getReadyConn(botId)` 等类名/函数名尚未完成 claw 重命名（待 UI 迁移阶段）。
...
| `BotConnection` | per-claw | 业务 RPC 抽象、连接等待、事件分发——**业务层唯一接口** |
