...
(per-tab 单例,                             (信令路由)               │
 多 connId)                                                        │
                    ─── WS /bots/stream ─► bot-ws-hub ◄──WS /bots/stream── Plugin
...
- **连接形态**：UI 以 `role=ui` 连接 bot-ws-hub（`WS /api/v1/bots/stream?role=ui`）
- **认证**：优先 session cookie，兼容一次性 ticket（`POST /api/v1/bots/ws-ticket` 生成，60s TTL）
