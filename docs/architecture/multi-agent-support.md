...
所有候选名称先经 `pick()` 过滤：值为 gateway 默认 `"Assistant"` 或等于 `agentId` 的占位名视为无信息量，不使用。过滤后按权威性 fallback：

```
agent.identity.get 的 name（resolvedIdentity.name）    ← 最权威，读取 IDENTITY.md（排除 "Assistant" 默认值）
  → agents.list 的 identity.name                       ← config identity 子对象
  → agents.list 的顶层 name                            ← 用户在 openclaw.json 中配置的 name
