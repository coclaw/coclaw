...
CoClaw 项目（尤其是 `openclaw-plugins/tunnel`）与 OpenClaw 生态紧密结合。在开发过程中如需了解 OpenClaw 的接口、配置结构或最佳实践，请优先查阅以下资源：
- 官方文档：位于 `/home/xhx/.nvm/versions/node/v22.21.1/lib/node_modules/openclaw/docs`。包含架构说明、API 参考与配置指南。
- 插件源码参考：位于 `/home/xhx/.nvm/versions/node/v22.21.1/lib/node_modules/openclaw/extensions`。
  - Telegram (`extensions/telegram`)：最佳的 Channel 插件实现参考（包含 `registerChannel`, `registerCommand`, 消息收发实现）。
  - Lobster (`extensions/lobster`)：复杂的 Tool 与流程编排参考。
  - LLM Task (`extensions/llm-task`)：Tool 注册与调用参考。
- 本地源码：
  - Github 仓库本地 clone 位于 `./openclaw-repo`。需要时可直接再本地查看 OpenClaw 项目的完整源码。
  - `/home/xhx/.nvm/versions/node/v22.21.1/lib/node_modules/openclaw`。可查阅 `openclaw.plugin.json` 规范及 TypeScript 类型定义等。
- 网络资源：
