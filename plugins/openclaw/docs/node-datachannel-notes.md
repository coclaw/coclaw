...
`cleanup()` 函数理论上需要在 gateway 停止时调用，否则 ThreadSafeCallback 可能阻止进程干净退出（ndc issue #366）。但 `cleanup()` 调用本身耗时 10s+，因此 `RealtimeBridge.stop()` 中**不调用** `cleanup()`（仅 `forceCleanup=true` 的测试场景除外），依赖 OS 在进程退出时回收资源。
...
已启用。`ndc-preloader.js` 中以 `"Warning"` 级别注册 `initLogger` callback，日志路由到 `remoteLog`（`ndc.native level=<level> <msg>`）。正常运行时零输出，仅故障时触发。注册失败不影响 ndc 正常使用。
