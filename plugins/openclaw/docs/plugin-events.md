...
| 事件名 | 触发时机 | payload 字段 |
|---|---|---|
| `coclaw.info.updated` | <ul><li>gateway connect 成功后由 `__pushInstanceInfo()` 调一次（基础三字段 + 采集成功时附带 `agentModels`；失败时省略以走 patch 语义保留旧值，见下文）</li><li>外线 server WS open 时若内线 `gatewayReady` 已 true 也补推一次（三线独立后内线可能先于外线就绪，第一次 push 在 server 路径被 drop）</li><li>`coclaw.info.patch` handler 改名成功后（仅 name + hostName）</li></ul> | `name?: string \| null` <br> `hostName?: string` <br> `pluginVersion?: string` <br> `agentModels?: Array<{id, name, model}> \| null` |
