...
- **caveat（性能 / 事件循环 — 2026-05-31 review 更正）**：**已采用 `readOnly:false`**——更新鲜（含新装厂商扩展）+ 含 manifest（才有 `openai-codex/*` 等 manifest-only provider）。**代价此前被低估**：`readOnly:false` 触发 discovery，其中 `discoverModels`（`ModelRegistry` 构造）对 `models.json` 是**同步 `readFileSync`+`JSON.parse`+schema 校验**、`discoverAuthStorage` 还同步清 legacy `auth.json`——这些会**卡网关事件循环**（OpenClaw 自己在 `model-catalog.ts` readOnly 分支注释点明 "provider discovery blocks the event loop"）。**关键**：网关原生 `models.list view:all` 走 `loadGatewayModelCatalog`（stale-while-revalidate 缓存 + readOnly 路套 750ms 超时兜底），过期先返旧、后台再刷，**永不卡当前请求**；但本插件是**裸调** `loadModelCatalog({readOnly:false})`，无此两层保护。故此前用 `openclaw gateway call models.list` 测出的 ~1.2s 测的是**有保护的网关路径**、代表不了插件裸路径，"`view:all` 同路径=非新增风险"的旧结论不成立。`loadModelCatalog` 自带的 `modelCatalogPromise` 模块缓存命中时快，但被网关 config reload 置空（`resetModelCatalogCache`，**含 `coclaw.model.set` 写配置触发的热重载**）→ 缓存失效后首次 `listUsable`/`set` 当场重扫、同步段卡网关（平时几十 ms 级、网关冷启最坏到秒级）。低频子页可接受，已知限制详见 `TODO.md` 该条 + 上游 #88392（available 信号落地后改读廉价源、绕开本路径）；指纹未变不写盘（fingerprint-gated + 原子写）。
