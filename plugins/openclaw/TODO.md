...
**影响**：多数情况只是噪音——对端连不上该私网地址，ICE 回落真 LAN/srflx/relay，浪费几次连通性探测。**真隐患**：若**对端也跑同类 VPN/utun 工具、落在同一 `172.19` 私网段**，可能凑出"伪 ICE pair"误判 P2P 成功（与代码里防 `docker0` 注释所述失败模式同源）。2026-06-15 本机实测：网关侧开 TUN 后 ICE 仍正确 nominate 真 LAN 候选（非 utun 伪候选），未观测到实际故障——属硬化项不是阻塞 bug。
