/**
 * runId → connId 路由表（独立基础设施，未集成进 bridge）
 *
 ...
 * - destroy 后 register / unregister / lookup / clear / init 全是 no-op
 * - register 写入策略：runId 不在表写入；同 reqId 刷新 expireAt；不同 reqId 跳过覆盖（首发优先）
 * - unregister 删除策略：runId 在表 + entry.reqId === 入参 reqId 才删（防跨 RPC 巧合 runId 误删）
...
export class RunEventRouter {
	...
	/**
	 * 注册路由条目。任一参数 falsy 静默返回（防御性）。
	 * @param {string} runId
	 * @param {string} connId
	 * @param {string} reqId
	 */
	register(runId, connId, reqId) {
		if (this.__destroyed) return;
		...
		if (existing && existing.reqId !== reqId) {
			...
			// logger.debug 自身抛错也不能让 register 失败（与 __scanExpired 内的 try/catch 风格一致）
			try { this.logger.debug?.(`[run-event-router] register skipped: runId already routed by reqId=${existing.reqId} new reqId=${reqId}`); }
			catch { /* logger 自身坏了不能让 register 抛 */ }
			return;
		}
	...
	/**
	 * 注销路由条目。runId 在表且 entry.reqId === 入参 reqId 才删。
	 * @param {string} runId
	 * @param {string} reqId
	 */
	unregister(runId, reqId) {
		if (this.__destroyed) return;
	...
	__scanExpired() {
		try {
			...
			if (cleaned > 0) {
				this.logger.warn?.(`[run-event-router] expired entries cleaned: count=${cleaned}`);
			}
		}
		catch {
