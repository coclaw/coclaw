...
- 2677ade: Restore the optional `openclaw` peerDependency so the plugin's `import('openclaw/plugin-sdk/*')` calls resolve under OpenClaw 2026.5.28's native plugin loader.
...
- 5a2a380: feat(plugin): add MiniMax OAuth login (device-code) provider-auth RPCs
