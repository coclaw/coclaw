...
- **runtime 没有等价 API 时**（如 `openclaw/plugin-sdk/provider-auth` 上游确实没暴露 `rt.providerAuth.*`），允许（并必须）import plugin-sdk 子路径，但严格遵循"字面量 specifier + 出现在插件入口源码里"的硬约束——详见 [`docs/openclaw-research/plugin-sdk-and-runtime.md`](../../docs/openclaw-research/plugin-sdk-and-runtime.md) 的"第三方插件如何 import Plugin SDK"章节。**Why:** 2026.5.28 起 loader 对编译成 `.js` 的入口**优先原生加载**（Node≥22.12 的 require(esm)）、绕过 jiti alias，Node<23.4 又没有 ESM 解析钩子，故入口里 `import('openclaw/plugin-sdk/*')` 须能原生解析到真 `node_modules/openclaw`，否则报 `Cannot find package 'openclaw'`。**故本仓库声明 `openclaw` 为 optional peerDependency**（上游契约：`openclaw plugins install` 据此建 `node_modules/openclaw → host openclaw` 软链、零传递依赖）；但 `--link` dev 安装**不**自动建链，且 install 期安全扫描禁 node_modules 外指软链，故由 `scripts/_lib.sh` 的 `ensure_openclaw_link`（在 `openclaw plugins install` **之后**调用，重启/加载 load.paths 时不再重扫故可存活）补；pnpm 开发侧锁膨胀（auto-install-peers 不认 optional）用根 `pnpm.overrides.openclaw → tools/openclaw-peer-stub` 空壳摁住。机制详见 [`docs/openclaw-research/plugin-sdk-and-runtime.md`](../../docs/openclaw-research/plugin-sdk-and-runtime.md)。
