...
> 本文件是相对根 AGENTS.md 的增量。仅写**硬约束**和**topic→doc 索引**——
...
- **runtime 没有等价 API 时**（如 `openclaw/plugin-sdk/provider-auth` 上游确实没暴露 `rt.providerAuth.*`），允许（并必须）import plugin-sdk 子路径，硬约束：**字面量 specifier + 出现在插件入口源码里**（loader 只扫入口字面量）。本仓库为此声明 `openclaw` 为 **optional peerDependency**；`--link` dev 安装不自动建 `node_modules/openclaw` 软链，由 `scripts/_lib.sh` 的 `ensure_openclaw_link` 在 `openclaw plugins install` **之后**调用补上；pnpm 开发侧用根 `pnpm-workspace.yaml` 的 `overrides`（`openclaw → link:./tools/openclaw-peer-stub` 空壳）摁住锁膨胀。loader 原生加载机制等原理详见 [`docs/openclaw-research/plugin-sdk-and-runtime.md`](../../docs/openclaw-research/plugin-sdk-and-runtime.md) 的"第三方插件如何 import Plugin SDK"章节。
...
- 当前 `claw-paths.js` 调上游 helper 时不传 `store` 配置和 sessions-index `entry`，目标永远是 OpenClaw 默认的 `<state-dir>/agents/<agentId>/sessions/...` 布局；honoring `session.store` / `entry.sessionFile` 两个上游覆盖旋钮是 follow-up（细节见 `TODO.md`）。
...
- **应对**：跨 hook/RPC 共享的状态**不能依赖纯内存缓存**，必须经磁盘中转——读取侧每次从磁盘 reload。现有 manager 的两种应对模式（磁盘中转 / 纯读不缓存）见 [`docs/module-boundaries.md`](docs/module-boundaries.md) 的单例清单。
...
- 所有 bind/unbind 核心逻辑集中在 `src/common/claw-binding.js`：RPC handler 与 `/coclaw bind` 斜杠命令共享 `doBind` / `doUnbind`。
...
- 阈值固化在 `package.json` 的 `test` 脚本（c8 参数），以其为准；未达标禁止安装到 gateway。
...
| 改 WebRTC 实现选择 / 看 `__ndcPreloadResult` 的命名困惑 / 想知道为何无兜底实现 | [`docs/webrtc-impl-strategy.md`](docs/webrtc-impl-strategy.md) |
...
| 改 topic 存储 / topic 长寿命存活（自管存档镜像）/ 非 main agent topic 支持 / OpenClaw 默认保洁两阶段删正文 | [`docs/topic-sessions-redesign.md`](docs/topic-sessions-redesign.md) |
