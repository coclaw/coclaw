...
3. **逐项过清单**：
   - 对必查清单 A-H 每一项：grep / read 上游相关文件，看是否有变更
...
7. **若发现新类别坑**（不在 A-H 之内）：按"演进规则"追加到清单。
...
- 不 commit / 不发布
- 不"顺手"修发现的不兼容
- 不在没跑完清单的情况下就出 PASS——清单 A-H 每一项都要明确给出 PASS / NEW-BLOCKER / NEW-WARNING
