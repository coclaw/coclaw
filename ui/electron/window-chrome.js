...
export function buildWindowChrome(platform, { forceNative = false } = {}) {
	if (forceNative) {
		return { custom: false, titleBarStyle: null, titleBarOverlay: null, backgroundColor: BACKGROUND_COLOR };
	}
	if (platform === 'darwin') {
		return { custom: true, titleBarStyle: 'hidden', titleBarOverlay: null, backgroundColor: BACKGROUND_COLOR };
	}
	if (platform === 'win32') {
		return {
			custom: true,
