...
- 0fd1200: Log send failures on both chat send paths (new-topic and established-chat) via `console.error` so remote troubleshooting can capture the error, and align the `InstanceOverview` channel-list wrapping with the shipped `ManageClawsPage` fix to avoid overflow on narrow screens.

## 0.32.7

### Patch Changes

- ba31d9e: Fix a stuck "streaming" indicator on an empty assistant bubble. When a run reaches its terminal state, the streaming placeholder flags are now cleared directly, so the spinner stops even if the follow-up message reload fails to drop the placeholder. This removes the stuck spinner seen when cancelling a reply before its first token, and the lingering "thinking" spinner left when a post-run message reload fails.
...
- 5883302: Restyle the Windows title bar brand to align with the sidebar icon column (24px logo with its left edge at 16px matching the nav icons, 14px text) instead of the Microsoft title bar metrics: the bar sits directly above the sidebar, so at that distance the standard metrics read as almost-but-not-aligned.

## 0.32.2

### Patch Changes

- f0768a5: Show the app logo + name on the left of the Electron custom title bar on Windows (following the Microsoft title bar design metrics), and hide the now-redundant sidebar brand row there, restoring the 8px top gap above the first nav item. macOS keeps the traffic-lights-only title bar and the sidebar brand, matching platform conventions.
...
- 1273702: Add provider OAuth device-code login UI and converge model-config on the three-capability model.
...
- 639fc03: Fix model-config page getting stuck on the loading spinner when switching to a
...
- 4ccf7e5: Fix model-config primary-model switch reverting to the old model after ~1-2s
...
- af44db1: fix(ui): unbind dialog dismiss + per-claw concurrency + 404 self-heal
...
- da05440: Defensive tolerance for plugin chat-history list anomalies in the chat history loader.
...
- 2a00e56: In `chatStore.__loadChatHistory`, filter entries returned from
...
- 7d0a685: fix(web-agents): open the list to anonymous users and clear it on identity change
...
- 3ecd5e9: fix(ui): clear orphan ended-run streaming placeholder after successful loadMessages
...
- 18d245f: feat(ui): show `<agentName>@<clawName>` label in ChatPage header
...
- 3a5a458: Fix mobile inability to pull older chat history when the current session is short. On a chat whose visible content does not overflow the scroll container (e.g. a freshly reset session with only the "New session started" marker), the existing scroll/wheel listeners that drive `__loadMoreHistory` cannot fire on touch devices: a finger pull at `scrollTop=0` produces no scroll event because there is no scrollable distance, and touch screens do not synthesize wheel events. Desktop is unaffected because mouse wheels fire `wheel` regardless of overflow. The fix attaches a small touch handler to the chat scroll container that activates only when the gesture starts at the very top, and on release calls `__loadMoreHistory` if the downward pull exceeded a 60px threshold — independent of whether the content can scroll. Normal touch scrolling is left to the browser (the handler does not preventDefault and only tracks gestures starting at the top), and the existing scroll/wheel paths continue to drive the desktop and overflow-scrollable cases.
  ...
  A follow-up review surfaced two narrow gaps in the above. First, the `chatStore` watcher's existing `if (this.__creatingTopic) return` early exit caused the new `__loadingHistory = false` reset to be skipped during the new-topic promote → `router.replace` window; combined with the new identity-gated `finally`, an in-flight `__loadMoreHistory` against the source store could leave `__loadingHistory` stuck at `true` on the post-promote topic page, blocking further pulls. Second, `__pullStartY` / `__pullDist` live on the component instance and were never cleared on chat switch, so a programmatic mid-touch chat switch could cause the subsequent `touchend` to trigger `__loadMoreHistory` on the new chat with a stale distance. Move the cleanup of all three transient fields above the `__creatingTopic` early exit, so any real chat-store change resets them regardless of whether activation is being suppressed.

- ea6348d: Defensive dedupe of duplicate `accepted` RPC frames in `ClawConnection.__handleRpcResponse`. The OpenClaw gateway protocol guarantees each agent RPC reqId only emits one `accepted` ack frame followed by one final res frame, but this guarantee lives entirely upstream. If a future protocol change or upstream bug ever delivers a second `accepted` on the same reqId, the existing UI handler would re-fire `onAccepted` side effects (refresh `__acceptedAt`, overwrite the agent run watcher, re-register in `agentRunsStore`, re-trigger `bumpActivity`, etc.). The new branch sets `waiter.__acceptedSeen` on the first ack and short-circuits any subsequent duplicate with `console.warn` plus `remoteLog('rpc.accepted.duplicate ...')` so the change is observable rather than silent. No effect on first-accepted handling, single-phase RPCs, final-frame resolution, timeout, or abort paths.
...
- 807c300: fix(ui): advance streaming anchor when stripping optimistic user msg
...
- 65ab93c: fix(ui): stop misjudging agent run as failed when agent.wait probe RPC rejects
