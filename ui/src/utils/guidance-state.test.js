...
describe('pickGuidanceState', () => {
	...
	describe('不再特判旧插件（旧插件凭据信号缺失 → 调用方传 false → 该弹就弹）', () => {
		test('旧插件无凭据信号当 hasAny=false → noKey（不再压制）', () => {
			// 旧插件给不出 hasAnyUsableCredential，调用方传 hasAny=false；以前会压制，现在直接 noKey
			expect(pickGuidanceState({ hasAny: false, primary: 'groq/llama', effective: false })).toBe('noKey');
		});

		test('旧插件主模型已设但 effective=false → invalid（不再压制）', () => {
			expect(pickGuidanceState({ hasAny: true, primary: 'groq/llama', effective: false })).toBe('invalid');
		});
	});
});
