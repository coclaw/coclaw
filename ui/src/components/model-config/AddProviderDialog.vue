...
// catalog authMethods 已知三类 + 固定渲染顺序（api-key 优先，其次设备码，最后 oauth-login 暂不支持）。
...
export default {
	...
	props: {
		 ...
		 * （api-key 输 key / oauth-device-code 设备码登录 / oauth-login 暂不支持）。
		 *
		...
		/**
		 * 发起两阶段 OAuth 设备码登录的回调，透传给 ProviderOAuthLoginStep（设备码入口用）。
	...
	methods: {
		 ...
		 * 回到 provider 选择。也复用为设备码/oauth-login 步的取消回退。
		 */
		onMethodBack() {
