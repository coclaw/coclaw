<template>
	<!--
		...
		Windows（platform==='win32'）在条左侧渲染品牌 logo+产品名——尺寸间距按微软 title bar 规范：
		icon 16×16 距左 16px、文字距 icon 16px、caption 12px；品牌不可点、随整条拖拽区（不加 no-drag）。
	...
	<div
		...
		aria-hidden="true"
	>
		<div v-if="showBrand" class="cc-titlebar-brand flex h-full items-center">
			<img :src="logoSrc" alt="" class="ml-4 size-4 rounded-xs" />
			<span class="ml-4 text-xs text-default">{{ $t('layout.productName') }}</span>
		</div>
	</div>
</template>
