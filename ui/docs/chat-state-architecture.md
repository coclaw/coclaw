...
```
SignalingConnection (per-claw, WebSocket)
  └─ 状态变化 → claws.store.__bridgeConn 桥接 → byId[clawId] 响应式字段
  
WebRtcConnection (per-claw, DataChannel)
  └─ event:agent / event:chat 事件流
        │
        ▼ (claws.store.__bridgeConn 统一注册)
agentRunsStore (全局单例)
...
`stores/claws.store.js`

```javascript
state: () => ({
	byId: {},       // { [clawId]: ClawState }
...
**`__bridgeConn(clawId)`**：在 `claws.store.js` 中注册，是唯一的连接状态桥接点。对每个 ClawConnection：
- 注入 `conn.__onGetRtcPhase` / `conn.__onTriggerReconnect` 回调
- 统一注册 `conn.on('event:agent', payload => agentRunsStore.__dispatch(payload))`
- Claw 上线且未初始化时触发 `__fullInit`（加载 agents + sessions + topics + dashboard）

**`getReadyConn(clawId)`**：导出的辅助函数，检查 `byId[clawId]?.dcReady` 并返回 ClawConnection 实例或 null。
...
| 数据 | Store | 离线行为 | 重连行为 |
|------|-------|---------|---------|
| Topic list | `topicsStore.byId` | 保留（增量合并，离线不清除） | `__refreshIfStale` / `__fullInit` 触发 `loadTopicsForClaw(id)`（per-claw） |
| Chat/topic content | `chatStoreManager` Map | 保留（实例留在 Map 中） | ChatPage `connReady` watcher 触发 silent refresh |
| Agent items | `agentsStore.byClaw` | 保留（离线不删除，重连后替换） | `__refreshIfStale` / `__fullInit` 触发 `loadAgents()` |
| Dashboard | `dashboardStore.byClaw` | 保留（离线不清除，重连后替换） | `__refreshIfStale` / `__fullInit` 触发 `loadDashboard()` |

### 后续候选

- **FileBrowser 目录列表**（`FileManagerPage` 组件 local data）：当前 unmount 即丢失。低频操作，优先级较低。如需缓存可提升到 store 层按 `${clawId}:${agentId}:${dir}` 为 key 存储
